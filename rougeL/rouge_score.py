...
def multi_rouge_n(sequences, scores_ids, n=2, exclusive=True):
    ...
    for hyp_id, ref_id in scores_ids:
        ...
        scores += [f_r_p_rouge_n(evaluated_count,
                                 reference_count, overlapping_count,beta=beta)]
    return scores


def rouge_n(evaluated_sentences, reference_sentences,
            n=2, raw_results=False, exclusive=True,beta=1.0):
    """
...
def f_r_p_rouge_n(evaluated_count, reference_count, overlapping_count,beta=1.0): #modified
    ...
    f1_score = (1+beta*beta) * ((precision * recall) / (beta*beta*precision + recall + 1e-8)) #modified
...
def rouge_l_summary_level(
        evaluated_sentences, reference_sentences, raw_results=False, exclusive=True,beta=1.0, **_):
    """
    ...
    f_lcs = (1.0+beta*beta) * ((p_lcs * r_lcs) / (beta*beta*p_lcs + r_lcs + 1e-8))
